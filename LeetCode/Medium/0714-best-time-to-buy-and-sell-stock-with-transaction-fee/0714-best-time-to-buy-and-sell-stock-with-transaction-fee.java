class Solution {
    ...
    at each sell we subtract transaction fees
   ...
   int transactionFees;
    ...
    public int f(int[] prices , int index , int bought, Integer[][]dp){
        ...
        int profit = 0;
        if(bought == 1){
            int sell = prices[index] + f(prices, index+1,0,dp) - transactionFees;    //skipped 1 index cause of cooldown
    ...
    public int tabulization(int[] prices){
        ...
        for(int index = n ; index>=0 ;index--){
            for(int bought = 0 ; bought <= 1 ; bought++){
                ...
                int profit = 0;
                if(bought == 1){
                    int sell = prices[index] + dp[index+1][0] - transactionFees;    //skipped 1 index cause of cooldown
