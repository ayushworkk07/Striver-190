/*  the common ground between both strings is LCS , so we will try to make both of them into
LCS , so that would take |n1 - lps| insertions/deletions and |n2-lps| insertions/deletions
*/
class Solution {
    public int minDistance(String word1, String word2) {
        int LPS = tabulization(word1,word2);

        return Math.abs(word1.length() - LPS) + Math.abs(word2.length() - LPS);
    }
